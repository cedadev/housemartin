...
First activate the ``housemartin`` Conda environment and install ``pytest``.
...
.. code-block:: console
  ...
  $ make clean
  $ make install
  $ conda list -n housemartin --explicit > spec-list.txt
...
Make a new version of housemartin in the following steps:
